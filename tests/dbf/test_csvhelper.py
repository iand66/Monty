...
def test_csvRead(filename, record):
    if os.path.exists(filename):
        assert len(csvRead(filename)) - 1 == record  # Allow for header record
...
def test_csvDictRead(filename, record):
    if os.path.exists(filename):
...
def test_csvWrite(get_db, temp, filename, table):
    assert (
        csvWrite(temp + "\\" + filename, dbSelect(get_db, table, **{"Id": "%"})) is True
    )
...
def test_csvDictWrite(get_db, temp, filename, table):
    assert (
        csvDictWriter(temp + "\\" + filename, dbSelect(get_db, table, **{"Id": "%"}))
        is True
    )
