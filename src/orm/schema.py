...
class Album(Base):
    ...
    DateCreated: Mapped[Date] = mapped_column(
        String(16), nullable=False, default=date.today().strftime("%Y-%m-%d")
    )
    DateUpdated: Mapped[Date] = mapped_column(
        String(16), nullable=False, default=date.today().strftime("%Y-%m-%d")
    )
...
class Employee(Base):
    ...
    ReportsTo: Mapped[int] = mapped_column(
...
class Track(Base):
    ...
    AlbumId: Mapped[int] = mapped_column(
        ForeignKey("Albums.Id"), nullable=False, index=True
    )
    MediaTypeId: Mapped[int] = mapped_column(
        ForeignKey("MediaTypes.Id"), nullable=False, index=True
    )
    GenreId: Mapped[int] = mapped_column(
        ForeignKey("Genres.Id"), nullable=False, index=True
    )
