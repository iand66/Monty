...
@router.get("/", summary='Get All Mediatypes', response_model=List[mediatypeCreate], status_code=status.HTTP_200_OK)
async def get_all(db: Session = Depends(get_db)) -> Any:
    ...
    **Get All Mediatypes:**
    ...
    if not result:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"No mediatype found")
    else:
        return result

@router.get("/id/{id:int}", summary= 'Get Mediatype by Id', response_model=List[mediatypeCreate], status_code=status.HTTP_200_OK)
...
@router.get("/name/{name:str}", summary='Get Mediatype(s) by Name', response_model=List[mediatypeCreate], status_code=status.HTTP_200_OK)
...
@router.post("/name/{name:str}", summary='Create New Mediatype', response_model=mediatypeCreate, status_code=status.HTTP_201_CREATED)
...
@router.put("/id/{id:int}", summary='Update Mediatype by Id', response_model=mediatypeCreate, status_code=status.HTTP_201_CREATED)
async def update_id(id: int, data: mediatypeCreate, db: Session = Depends(get_db)) -> Any:
    """
...
@router.put("/name/{name:str}", summary='Update Mediatype by Name', response_model=mediatypeUpdate, status_code=status.HTTP_201_CREATED)
async def update_name(name: str, data: mediatypeUpdate, db: Session = Depends(get_db)) -> Any:
    """
...
@router.delete("/id/{id:int}", summary='Delete Mediatype by Id', response_model=List[mediatype], status_code=status.HTTP_202_ACCEPTED)
...
@router.delete("/name/{name:str}", summary='Mediatype(s) by Name', response_model=List[mediatype], status_code=status.HTTP_202_ACCEPTED)
