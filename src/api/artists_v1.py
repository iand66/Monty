...
@router.get(
    ...
    response_model=List[artistCreate],
...
async def get_id(id: int, db: Session = Depends(get_db)) -> Any:
    ...
    **Get Artist by Artist Id:**
    ...
    if not result:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Artist {id} not found"
        )
    else:
        return result
...
@router.get(
    "/name/{name:str}",
    summary="Get Artists(s) by Artist Name",
    response_model=List[artistCreate],
    status_code=status.HTTP_200_OK,
)
async def get_name(name: str, db: Session = Depends(get_db)) -> Any:
    ...
    **Get Artist by Artist Name:**
    ...
    if not result:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Artist {name} not found"
        )
    else:
        return result
...
async def create_name(data: artistCreate, db: Session = Depends(get_db)) -> Any:
    ...
    **Create New Artist:**
    ...
    if result:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail=f"Artist {data} already exists"
        )
    else:
...
@router.put(
    "/id/{id:int}",
    summary="Update Artist by Artist Id",
    response_model=artistCreate,
    status_code=status.HTTP_201_CREATED,
)
...
@router.put(
    "/name/{name:str}",
    summary="Update Artist by Artist Name",
    response_model=artistUpdate,
    status_code=status.HTTP_201_CREATED,
)
async def update_name(
    name: str, data: artistUpdate, db: Session = Depends(get_db)
) -> Any:
    """
...
@router.delete(
    "/id/{id:int}",
    summary="Delete Artist by Artist Id",
...
@router.delete(
    "/name/{name:str}",
    summary="Artist(s) by Artist Name",
    response_model=List[artist],
    status_code=status.HTTP_202_ACCEPTED,
)
