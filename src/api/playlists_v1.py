...
@router.get(
    ...
    response_model=List[playlistCreate],
...
async def get_all(db: Session = Depends(get_db)) -> Any:
    ...
    **Get All Playlists:**
    ...
    if not result:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No playlists found"
        )
    else:
        return result
...
async def get_id(id: int, db: Session = Depends(get_db)) -> Any:
    ...
    **Get Playlist by Playlist Id:**
    ...
    if not result:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Playlist {id} not found"
        )
    else:
        return result
...
@router.get(
    "/name/{name:str}",
    summary="Get Playlist(s) by Playlist Name",
    response_model=List[playlistCreate],
    status_code=status.HTTP_200_OK,
)
async def get_name(name: str, db: Session = Depends(get_db)) -> Any:
    ...
    **Get Playlist by Playlist Name:**
    ...
    if not result:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Playlist {name} not found"
        )
    else:
        return result
    ...
    **Create New Playlist:**
    ...
    if result:
        raise HTTPException(
            ...
            detail=f"Playlist {data} already exists",
        )
    else:
...
@router.put(
    "/id/{id:int}",
    summary="Update Playlist by Playlist Id",
    response_model=playlistCreate,
    status_code=status.HTTP_201_CREATED,
)
async def update_id(
    id: int, data: playlistCreate, db: Session = Depends(get_db)
) -> Any:
    """
...
@router.put(
    "/name/{name:str}",
    summary="Update Playlist by Playlist Name",
    response_model=playlistUpdate,
    status_code=status.HTTP_201_CREATED,
)
async def update_name(
    name: str, data: playlistUpdate, db: Session = Depends(get_db)
) -> Any:
    """
...
@router.delete(
    "/name/{name:str}",
    summary="Playlist(s) by Playlist Name",
    response_model=List[playlist],
    status_code=status.HTTP_202_ACCEPTED,
)
